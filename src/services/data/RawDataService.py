...
from models.raw.pipe.RawPipe import RawPipe
from models.raw.pipe.RawPoint import RawPoint
...
class RawDataService:
    ...
    def import_address_data(self):
        with fiona.open(
                os.getcwd() + '\\resources\\inputs\\Adressen.gpkg') as layer:
            for obj in layer:
    ...
    def import_height_data(self):
        dataset = pd.read_json(os.getcwd() + '\\resources\\inputs\\onegeo.json')
        elements = dataset['features']
    ...
    def import_pipe_data(self):
        with open(os.getcwd() + '\\resources\\inputs\\Rohrleitungen.geojson') as f:
        ...
        for pipe_obj in sorted_elements:
            ...
            path_object = RawPipe(length, json.loads(to_geojson(LineString(coordinates))))
    ...
    def get_raw_building_data(self):
        raw_building_data = DataFrame.from_records(vars(o) for o in self.rdr.raw_thermos_data)
        ...
        return [raw_building_data, raw_address_data, raw_height_data]

    def import_data(self):
        self.dbs.create_table_from_class(RawBuildingThermosData)
        self.dbs.create_table_from_class(RawBuildingAddress)
        self.dbs.create_table_from_class(RawBuildingHeight)
        self.dbs.create_table_from_class(RawPoint)
        self.dbs.create_table_from_class(RawPipe)
